...
class make_listpage_referrors:
	...
	def __init__(self, tpls_like_sfns_names, filename_tpls_transcludes):
		...
		if get_transcludes_from == 1:  # 'connect'
		...
		elif get_transcludes_from == 2:  # 'file'
			self.list_transcludes = file_readlines_in_set(filename_tpls_transcludes)
		elif get_transcludes_from == 3:  # 'manual'
			self.list_transcludes = ['Раскраска_графов', 'Звёздчатый_октаэдр']  # тест отдельных страниц
